...
use crate::primitives::{DynamicState, PolyType, Primitives};
use crate::skeleton::{SMatrix, Skeleton, Transform};
use crate::util::{BiMap, BiVec};
use cgmath::Matrix4;
use json::JsonValue;
...
fn mesh(ctx: &Ctx, gltf: &mut GlTF) {
    ...
    gltf.json["extensionsUsed"]
        .push("FB_ngon_encoding")
        .unwrap();
}
...
fn nodes(ctx: &Ctx, gltf: &mut GlTF) {
    ...
    gltf.json["nodes"] = ctx
        ...
        .map(|idx| {
            ...
            let children = ctx.skel.tree.children(idx).collect::<Vec<_>>();
...
fn animations(ctx: &Ctx, gltf: &mut GlTF) {
    ...
    let mut animations = models_animations
        .iter()
        .map(|&animation_id| {
    ...
    for mat_anim_conn in mat_animations {
        ...
        for track in &mat_anim.tracks {
            ...
            // Find the target
            let material_idx = model
                .materials
                .iter()
                .position(|mat| mat.name == track.name)
                .unwrap();
            let target = format!(
            ...
            let (w, h) = (
                model.materials[material_idx].width as f64,
                model.materials[material_idx].height as f64,
            );
...
fn materials(ctx: &Ctx, gltf: &mut GlTF) {
    ...
    // Embed each texture as a PNG inside the GLB buffer so the model is
    // self-contained (upstream apicula references external .png files).
    {
        let rev: HashMap<&str, (usize, Option<usize>)> = ctx
            .image_namer
            .names
            .iter()
            .map(|(k, v)| (v.as_str(), *k))
            .collect();
        let names: Vec<String> = image_descs.iter().cloned().collect();
        let mut images_json: Vec<JsonValue> = Vec::with_capacity(names.len());
        for name in &names {
            let mut embedded = None;
            if let Some(&(tex_id, pal_id)) = rev.get(name.as_str()) {
                let texture = &ctx.db.textures[tex_id];
                let palette = pal_id.map(|id| &ctx.db.palettes[id]);
                if let Ok(rgba) = crate::nds::decode_texture(texture, palette) {
                    ...
                    let mut png_bytes = Vec::new();
                    {
                        ...
                        if let Ok(mut wr) = enc.write_header() {
                            let _ = wr.write_image_data(&rgba.0);
                        }
                    }
                    if !png_bytes.is_empty() {
                        let buf_idx = gltf.buffers.len();
                        let len = png_bytes.len();
                        gltf.buffers.push(Buffer {
                            bytes: png_bytes,
                            alignment: 1,
                        });
                        let bv = gltf.json["bufferViews"].add(object!(
                            "buffer" => buf_idx,
                            "byteOffset" => 0,
                            "byteLength" => len,
                        ));
                        embedded = Some(object!(
                            "mimeType" => "image/png",
                            "bufferView" => bv,
                        ));
                    }
            ...
            images_json.push(embedded.unwrap_or_else(|| object!("uri" => format!("{}.png", name))));
        }
        gltf.json["images"] = images_json.into();
    }
