...
use crate::nitro::{
    Animation, Container, MaterialAnimation, Model, Name, Palette, Pattern, Texture,
};
...
impl Database {
    /// Build a database from raw Nitro file buffers (e.g. NSBMD/NSBTX blobs
    /// pulled from a ROM or NARC). Buffers that do not parse as a Nitro
    /// container are skipped.
    pub fn from_buffers(buffers: Vec<Vec<u8>>) -> Database {
        use crate::nitro::container::read_container;

        let mut db: Database = Default::default();
        db.file_paths = (0..buffers.len())
            .map(|i| PathBuf::from(format!("buffer{i}")))
            .collect();
        for (file_id, buf) in buffers.iter().enumerate() {
            if let Ok(cont) = read_container(Cur::new(buf)) {
