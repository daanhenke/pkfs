...
use crate::db::{AnimationId, Database, MatAnimId, ModelId, PaletteId, PatternId, TextureId};
...
impl MaterialConnection {
    ...
    pub fn texture_id(&self) -> Option<TextureId> {
        match *self {
            ...
            MaterialConnection::TextureOkNoPalette { texture }
            | MaterialConnection::TextureOkPaletteMissing { texture }
            | MaterialConnection::TextureOkPaletteOk { texture, .. } => Some(texture.id),
        }
    }
    ...
    pub fn palette_id(&self) -> Option<TextureId> {
        match *self {
            ...
            | MaterialConnection::TextureOkPaletteMissing { .. } => None,
            MaterialConnection::TextureOkPaletteOk { palette, .. } => Some(palette.id),
        }
    }
    ...
    db.patterns
        ...
        .filter_map(|(pattern_id, pattern)| {
            ...
            let valid = pattern
                .material_tracks
                .iter()
                .all(|track| model.materials.iter().any(|mat| mat.name == track.name));
            if !valid {
                return None;
            }
...
fn find_applicable_mat_anims(db: &Database, model_id: ModelId) -> Vec<MatAnimConnection> {
    ...
    db.mat_anims
        .iter()
        .enumerate()
        .filter_map(|(mat_anim_id, mat_anim)| {
            // Check if all the tracks target valid materials
            let valid = mat_anim
                .tracks
                .iter()
            ...
            if !valid {
                ...
                Some(MatAnimConnection { mat_anim_id })
            }
        })
        .collect()
}
