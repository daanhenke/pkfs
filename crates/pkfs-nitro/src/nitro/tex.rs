...
use crate::nds::{TextureFormat, TextureParams};
...
pub fn read_tex(cur: Cur) -> Result<(Vec<Texture>, Vec<Palette>)> {
    ...
    let pal_block = (cur + pal_block_off)
        .next_n_u8s(pal_block_len)?
        .to_vec()
        .into_boxed_slice();
    ...
    let textures = info_block::read::<(u32, u32)>(cur + texture_off)?
    ...
    let palettes = info_block::read::<(u16, u16)>(cur + palette_off)?
